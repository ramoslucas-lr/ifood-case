...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--silver-path",
        required=True,
        help="Caminho absoluto do S3 para gravar na Silver",
    )
    parser.add_argument(
        "--table-name",
        required=True,
    ...
    parser.add_argument(
        ...
        required=False,
        default="",
        help="Chaves de partição separadas por vírgula (ex: ano,mes)",
    )
    parser.add_argument(
        "--partition-values",
        required=False,
        default="",
        help="Valores de partição separados por vírgula (ex: 2023,01)",
    )
...
def main() -> None:
    ...
    try:
        ...
        logger.warning(
            f"Erro ao ler dados da camada Bronze no caminho '{bronze_path}'."
        )
        logger.warning(f"Detalhes: {str(e)}")
        logger.warning(
            "Finalizando execução com sucesso para evitar quebra da pipeline em partições inexistentes."
        )
        return
    ...
    logger.info(
        f"Processamento finalizado com sucesso. Dados gravados na Silver em {silver_path}"
    )
