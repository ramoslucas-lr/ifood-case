...
cwd = os.getcwd()
current_dir = (
    cwd if cwd.endswith("src/databricks") else os.path.join(cwd, "src", "databricks")
)
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--mart-name",
        required=True,
        help="Nome lógico do Data Mart a ser gerado (ex: monthly_revenue)",
    )
    parser.add_argument(
        "--silver-path",
        required=True,
        help="Caminho absoluto do S3 para ler a Fato da Silver",
    )
    parser.add_argument(
        "--gold-path", required=True, help="Caminho absoluto do S3 para gravar na Gold"
    )
    parser.add_argument(
        "--table-name",
        required=True,
        help="Nome da tabela no Unity Catalog/Hive Metastore",
    )
    parser.add_argument(
        "--partition-keys",
        required=False,
        default="",
        help="Chaves de partição em processamento (ex: ano,mes)",
    )
    parser.add_argument(
        "--partition-values",
        required=False,
        default="",
        help="Valores da partição em processamento (ex: 2023,01)",
    )
...
def main() -> None:
    ...
    part_keys = (
        [k.strip() for k in args.partition_keys.split(",")]
        if args.partition_keys
        else []
    )
    part_values = (
        [v.strip() for v in args.partition_values.split(",")]
    ...
    try:
        ...
        logger.warning(
            "Finalizando execução com sucesso para evitar quebra da pipeline."
        )
        return
    ...
    if part_keys:
        replace_condition = " AND ".join(
            [f"{k} = '{v}'" for k, v in zip(part_keys, part_values)]
        )
    else:
    ...
    logger.info(
        f"Processamento finalizado com sucesso. Data Mart gravado em {gold_path}"
    )
