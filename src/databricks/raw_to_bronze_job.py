...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--bronze-path", required=True, help="Caminho raiz da tabela na Bronze"
    )
    ...
    parser.add_argument(
        ...
        required=False,
        default="",
        help="Chaves de partição (ex: ano,mes)",
    )
    parser.add_argument(
        "--partition-values",
        required=False,
        default="",
        help="Valores da partição (ex: 2023,01)",
    )
...
def main() -> None:
    ...
    try:
        ...
        existing_schema = {
            f.name.lower(): f.dataType for f in existing_df.schema.fields
        }
    ...
    logger.info(
        f"Processamento Raw->Bronze finalizado. Dados gravados em {args.bronze_path}"
    )
